...
class Ventana_nuevo(tk.Toplevel):
    ...
    def campos(self):
        ...
        self.btn_crear.config(width=15, bg="#2B7984", command=self.crearVuelo)
    ...
    def crearVuelo(self):
        ...
        crear = crearVuelo(self.entry_origen.get(),self.entry_destino.get(),self.entry_horario.get(),self.entry_precio.get(),self.entry_asiento.get())
        self.listar()
    ...
    def cerrar_y_recargar(self):
        self.destroy()
        if self.master:  # Asegura que hay una ventana principal (master)
            self.master.listar()